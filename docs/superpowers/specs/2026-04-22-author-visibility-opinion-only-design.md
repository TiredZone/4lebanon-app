...
Today, author names are hidden only on breaking articles (commit `62fda4f` wired this to the `is_breaking` flag). The editorial decision has shifted: author credit should be an opinion-column convention, not a general news convention. We want the byline to appear exclusively on pieces in the opinion section (`رأي` / slug `opinion`) and be suppressed everywhere else — news, breaking, analysis, geopolitics, economy, security, special, and any future non-opinion section.
...
else if article.section.slug == 'opinion' → show
else                             → hide
```
...
New body: return `null` if the author is missing, anonymous, or `sectionSlug !== 'opinion'`.

The opinion slug is a constant in one place (`'opinion'`). Define it as an exported constant in `lib/utils.ts` (e.g. `OPINION_SECTION_SLUG`) so call sites and queries reference the same value.
...
with a filter that restricts results to articles whose section slug is `'opinion'`. In Supabase this is typically done by joining `sections` and filtering on `sections.slug`, or by looking up the opinion section id once and using `.eq('section_id', opinionSectionId)`.
...
2. **Opinion article (section = `opinion`, author assigned, not anonymous):** byline appears in all five surfaces plus RSS.
3. **Opinion article with anonymous author:** no byline anywhere.
4. **Breaking opinion article** (priority 2 + section = `opinion`): byline shown (confirms `is_breaking` no longer gates visibility). The "عاجل" badge still renders.
...
7. **RSS feed:** re-fetch `/rss.xml` and confirm `<author>` tags appear only on items whose `<category>` is `رأي`.
